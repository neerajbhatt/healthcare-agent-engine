"""Provider Behavior Agent — profiles providers against peers, flags outliers."""
...
class ProviderAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            agent_id="provider",
            agent_name="Provider Behavior Agent",
            domain="provider",
            wave=1,
        )
    ...
    async def execute(self, context: dict[str, Any]) -> AgentResult:
        ...
        analysis_prompt = f"""Analyze the following provider data for NPI {params['provider_npi']}
...
Focus on:
1. Whether the provider is a statistical outlier vs peers (z-score > 2 is notable, > 3 is critical)
2. Whether there are specialty mismatches (billing for out-of-specialty procedures)
3. Unusual patient panel composition
